...
### Added
- **`ledger_upsert_party`** — a fourteenth `ledger_*` tool that creates or updates a customer/vendor through the unified port (both adapters already implemented `upsertParty`: Merit maps it to `sendcustomer`, e-arveldaja to client create/update). Pass the party's `id` Ref to update, omit it to create. The default surface is now 147 tools (133 e-arveldaja + 14 `ledger_*`; 142 with Lightyear disabled).
- **Audit logging for mutating `ledger_*` tools** — `ledger_upsert_party`, `ledger_create_sales_invoice`, `ledger_create_purchase_invoice`, `ledger_record_payment`, `ledger_post_journal`, `ledger_confirm`, and `ledger_void` now write the session audit log like every other mutating tool. Writes against a non-e-arveldaja backend go to that backend's own file (`logs/merit.audit.md`) so another system's mutations are never attributed to an e-arveldaja company; non-numeric backend ids (Merit GUIDs) ride in `details.backend_ref`. `ledger_confirm` is logged only on explicit-booking backends (on auto-post backends it is a status read that mutates nothing), and `ledger_void` logs `INVALIDATED` on e-arveldaja vs `DELETED` on Merit, matching what each backend actually does.
- **First-class Merit-only ledger session + full ledger CRUD surface** — the ledger layer is now usable as a standalone backend, not just a sidecar to e-arveldaja:
  - **Six `ledger_list_*` read tools** (`ledger_list_accounts`, `ledger_list_tax_rates`, `ledger_list_parties`, `ledger_list_items`, `ledger_list_sales_invoices`, `ledger_list_purchase_invoices`) so a non-e-arveldaja backend can be read back, not just blind-written.
  - **Three more write/lifecycle tools** — `ledger_create_purchase_invoice`, `ledger_confirm`, and `ledger_void` round out the cross-backend surface (13 `ledger_*` tools total). On an auto-post backend like Merit, `ledger_confirm` reports steady state and `ledger_void` deletes; on e-arveldaja they register / invalidate.
  - **Honest discovery** — `list_ledger_backends` now reports e-arveldaja's *real* credential state instead of always claiming configured, and default-backend resolution falls back to the first configured backend when e-arveldaja has no credentials, so unqualified `ledger_*` calls route to Merit in a Merit-only setup.
  - **Ledger session at boot** — when e-arveldaja has no credentials but a ledger backend (Merit) is configured, the server boots as a "ledger session": the startup banner and server instructions say the configured backend is available and the `ledger_*` tools work against it, instead of presenting the whole server as a broken e-arveldaja setup. The e-arveldaja-specific tools still require e-arveldaja credentials.- **Multi-backend ledger abstraction (`src/ledger/`) + unified `ledger_*` tools** — an intermediate ports-and-adapters layer that lets the server drive other Estonian bookkeeping systems through one canonical model, **Merit Aktiva** first. A backend-neutral `LedgerConnector` port (with a capability descriptor covering booking model, numbering, dimensions, VAT scope, source-doc requirement, and feature flags) sits over two adapters: an `EarveldajaAdapter` that wraps the existing `ApiContext` api clients (the explicit PROJECT→CONFIRMED booking model, unchanged behaviour), and a `MeritAdapter` (HMAC-SHA256 signing verified against Merit's published test vector; the auto-post booking model where documents post on create and `confirm()` is a no-op). A registry exposes whichever backends are configured. The `ledger_*` MCP tools route through the port so an agent can target either backend with one vocabulary. The existing e-arveldaja-specific tools are untouched; the ledger tools are additive. Merit is enabled by setting `MERIT_API_ID` / `MERIT_API_KEY` (and optionally `MERIT_API_COUNTRY=EE|PL`); without them only e-arveldaja is registered. Default backend is selectable via `EARVELDAJA_LEDGER_DEFAULT_BACKEND`. An opt-in live smoke test (`src/__integration__/merit-adapter.integration.test.ts`) runs read-only Merit calls when credentials are present. See `ARCHITECTURE.md` → "Ledger abstraction layer".

### Fixed
- **The server no longer crashes at startup when the MCP client launches it in an unwritable working directory** — `initAuditLog()` created `logs/` under `process.cwd()` and let the error escape, so a client that starts the server from `/` (Claude Desktop does) died immediately with `Fatal: ENOENT: no such file or directory, mkdir '/logs'` before a single tool was registered. `ensureLogsDir()` is now non-throwing: it falls back to `logs/` inside the per-user global config directory — the same place credentials already fall back to — and warns on stderr; if neither location can be created, audit logging is disabled with a loud warning rather than taking the server down. A new `getAuditLogsDir()` reports where entries actually land. Regression test boots the module with an unwritable cwd and asserts the fallback path, the written entry, and the warning.
- **Merit adapter validated against the official Merit Aktiva API reference** (<https://api.merit.ee/connecting-robots/reference-manual/>) — a documentation pass, captured in the new `docs/merit-api-notes.md`, confirmed the auth/signing, base URLs, endpoint versions, and reference-data field names, and corrected the remaining discrepancies:
  - **`TotalAmount` is the NET total ("Amount without VAT"), not gross.** The official create-invoice spec labels it net for both sales and purchase invoices (Merit derives the gross `TotalSum` itself), but the adapter had been sending gross on the strength of the reference client's example — which would **double-count VAT on a VAT-registered company**. Now sends net (Σ quantity × price); `raw.TotalAmount` overrides. (The demo company posts 0% VAT, so net == gross there — which is why no test had caught it.)
  - **Vendor/customer required-on-create fields.** Per the official create pages, a new vendor needs `VatAccountable` + `CountryCode` and a new customer needs `NotTDCustomer` + `CountryCode`. The adapter now defaults these on create (VatAccountable inferred from a VAT number, CountryCode `EE`, NotTDCustomer false), all overridable via `raw`; on update they are omitted.
  - **Rate limit corrected to the documented 100 requests/minute** (was described as ~60 in comments; the ~1 req/s pacing stays well under it). Documented the 500-row document cap and the 3-month list-query limit (server-enforced: `"Periood liiga pikk(max 3 kuud)"`).
  - Documented the **`sendpayment` customer-receipt endpoint** (CustomerName + InvoiceNo, payable in parts) as the way to close the "receivables cannot be recorded through the port yet" gap in the reconcile-bank / import-camt ledger branches — a wired follow-up, not yet implemented.
  - Re-verified live: the 12-case Merit E2E write cycle still passes end-to-end with the net-total and vendor-default changes.
- **Merit adapter parity pass against the jaakla/merit_api reference client** — a systematic comparison with the live-tested Python implementation (plus read-only probes of the live API) surfaced and fixed a batch of payload/response bugs that unit tests with self-consistent mocks could not catch:
  - **Purchase invoices**: rows go under `InvoiceRow` (was the invented `PurchaseInvoiceRow`), the row account field is `GLAccountCode` (was `Account` — that name is sales-only), `Vendor` must carry both `Id` AND `Name` (the adapter now resolves the name from the vendor registry for Ref-only vendors and fails with `not_found` when absent), and the payload now includes the required `TransactionDate`, `CurrencyRate`, `TaxAmount`, `TotalAmount`, and `RoundingAmount`. `sourceDocument` now maps to Merit's `Attachment {FileName, FileContent}`. The create response field is `BillId` (was `PurchInvoiceId` — refs came back empty), and list rows likewise map `BillId`.
  - **VAT amounts**: `TaxAmount` now carries the real per-rate VAT (was always 0) and `TotalAmount` is the gross sum (was net) — a 24% invoice previously posted with wrong totals. The tax cache stores rate percentages alongside GUIDs to compute this.
  - **Payments**: `sendPaymentV` takes a flat vendor-payment document (`BillNo` + `VendorName` + `IBAN` + `PaymentDate` + `Amount`, v2 when `CurrencyCode` present) — the previous `PaymentRow` array shape was invented. The adapter resolves `BillNo`/`VendorName` from a `purchaseInvoice` ref via `getpurchorder` and the IBAN from the vendor registry (raw fields take precedence), settles one bill per call, and reads the response id from `InvoiceId`.
  - **Vendors are now visible**: Merit keeps customers and vendors in separate registries; `listParties` now merges `getcustomers` + `getvendors` (previously vendors were completely absent) and `ledger_upsert_party` routes `kind:"vendor"` to `sendvendor`. Live row id fields are `CustomerId`/`VendorId`/`ItemId` (not `Id`); customer/vendor `BankAccount`→`iban` and `Email` are mapped.
  - **Reads**: `getcustomers`/`getitems` moved to v1 (reference-client default; item names live in `Name`, units in `UnitofMeasureName`), list endpoints get a default last-90-days period (Merit requires one and caps at ~3 months), and the next-invoice-number lookup now bounds its query window to the same cap.
  - **Transport**: Merit reports business errors as HTTP 200 + `{"Success": false, …}` — these are now thrown (a rejected write can no longer masquerade as a created document); 5xx responses are retried once on idempotent `get*` endpoints; canonical `raw` objects (invoice, line, payment) are spread into outgoing payloads last, making the documented raw escape hatch actually work; and the Polish host is corrected to `program.360ksiegowosc.pl`.
  - Live smoke test extended to parties and items with non-empty-ref assertions.
- **Merit chart-of-accounts mapping** — live verification against a real Merit company revealed `getaccounts` returns `AccountID` (not `Id`) and exposes no account-type or dimension fields, so every canonical account ref was silently mapped to an empty string. The adapter now reads `AccountID`, filters `NonActive` rows (parity with the e-arveldaja adapter's `is_deleted` filter), and omits `type` / `requiresDimension` instead of guessing — both are now optional on the canonical `Account`, per the "declare, don't fake" capability rule. The live smoke test now asserts every account carries a non-empty ref and code, so an upstream field rename fails loudly instead of degrading silently.

### Added
- **Ledger branches for ALL 15 workflow prompts + a live Merit E2E suite covering them** — the remaining 12 prompts gained graded backend routing (every recipe now serves its runbook in a ledger session):
  - Full ledger branches: `receipt-batch` (scan + OCR locally, one consolidated approval, `ledger_create_purchase_invoice` per receipt), `import-camt` (parse locally, settle open payables via `ledger_record_payment`), `reconcile-bank` (settle-status review + payment recording), `month-end-close` (lighter close review from list reads incl. duplicate `vendorBillNo` detection), `lightyear-booking` (same local parsing, journals via `ledger_post_journal` with `LY:` keys in `docNo`), `accounting-inbox` (local scan, follow-ups routed to each workflow's ledger branch).
  - Limited/handoff branches: `import-wise` (treat the CSV as a statement, follow the import-camt ledger branch), `classify-unmatched` (nothing to classify on auto-post backends — hands off to reconcile-bank), `resolve-accounting-review` / `prepare-accounting-review-action` (hand ledger-expressible treatments to the matching workflow's ledger branch).
  - **New E2E suite** `merit-e2e-usecases.integration.test.ts`: runs the real MCP server as a Merit-only ledger session against a live Merit company and exercises all 15 use cases — prompt routing, local scan tools, live reads, and (behind `MERIT_E2E_WRITE=1`) a full write cycle: vendor upsert → 0.01 EUR purchase invoice → payment settling it → balanced GL batch → sales invoice created and voided → per-backend audit-log verification. All 12 tests pass live.
  - Live E2E findings fixed along the way: `sendvendor` is a **v2** endpoint (v1 404s; the reference client's default is wrong), Merit **requires an item code on every invoice row** (now a clear validation error instead of an Estonian 400), invoice list rows carry `SIHId`/`PIHId` + `DocumentDate`/`TotalSum`/`Paid` (settle status now uses them), `getpurchorder` nests the invoice under `Header`, and `Payment.bank` now accepts a GL account code / bank name / BankId (resolved via `getbanks` — not every bank row is a valid payment bank).
  - **MCP interop fix:** prompt arguments arrive as strings per the MCP spec, but several prompt schemas used `z.number()`/`z.boolean()` — `lightyear-booking` (required numeric args) was uncallable from spec-compliant clients. All typed prompt args now coerce (`z.coerce.number()`, a strict string-boolean preprocess).

### Changed
- **Workflow prompts split into backend-aware vs e-arveldaja-only** — the 15 MCP prompts now declare their backend scope explicitly instead of implicitly assuming e-arveldaja:
  - **Backend-aware** (`book-invoice`, `new-supplier`, `company-overview`): each recipe opens with a Step 0 that targets e-arveldaja by default and routes to a **ledger branch** (`ledger_list_*`, `ledger_upsert_party`, `ledger_create_purchase_invoice`) when the user names another backend or the server runs as a ledger session. `book-invoice`'s local OCR/validation steps apply on both paths; its ledger branch does its duplicate check via `ledger_list_purchase_invoices`, warns that auto-post backends post on create (no confirm step), and passes the source document as the canonical `sourceDocument`. `company-overview` produces a lighter list-based overview where port reporting is not exposed. In a **ledger session** these three prompts now serve their real runbook instead of the setup-mode text (`registerPrompts` gains a `ledgerSession` flag).
  - **e-arveldaja-only** (the other 10 workflow prompts + both setup prompts): each now states **"Backend: e-arveldaja only"** with the specific native machinery it depends on, and instructs the agent to stop in a ledger session rather than improvise an equivalent with `ledger_*` tools. The setup prompts point to the Merit env vars (`MERIT_API_ID`/`MERIT_API_KEY`) for non-e-arveldaja backends.
  - Also fixed stale guidance in `new-supplier`: `create_client` now documents the required `is_physical_entity` (the removed `is_juridical_entity` was still referenced).
- **Merit transport hardening** — `MeritHttpClient` now paces requests to ~1/second (Merit throttles per API key, ~60 requests/minute) and retries once: any endpoint on 429 (the request was rejected before processing), and only idempotent `get*` endpoints on network errors — every Merit call is a POST, so a network failure after a `send*` may have reached Merit and a blind retry could double-post a document.
- **OCR (`@llamaindex/liteparse`) is now loaded lazily** — `src/document-parser.ts` uses a type-only import and `import()`s the native module on first OCR use instead of at module load. Because liteparse is a Rust native addon (bundling PDFium) with a prebuilt binary per platform, the previous eager import crashed the **entire server at startup** on any host where the binary cannot load — notably Linux with glibc < 2.38 and musl/Alpine — even for flows that never OCR anything (ledger/Merit, CRUD, reports). Now the server boots everywhere; only the OCR tools (`extract_pdf_invoice`, receipt batch, …) fail, and with a clear actionable message ("Document OCR is unavailable on this platform … requires macOS, Linux x64/arm64 with glibc ≥ 2.38, or Windows x64; musl/Alpine unsupported") instead of a cryptic native stack trace. This also makes the Merit-only ledger session and the integration tests runnable on more platforms/CI.
